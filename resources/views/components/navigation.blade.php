...
<script>
    ...
    $('#collapse').click(function(event){
        ...
        if (!isSidebarOpen) {
            ...
            sidebar.stop().css({ 'right': '-250px', 'display': 'block' }).animate({ 'right': '0px' }, 'slow');
        } else {
            //de izquierda a derecha
            sidebar.stop().animate({ 'right': '-250px' }, 'slow', function() {
                sidebar.css('display', 'none'); //esconder la sidebar cuando la animacion acabe
            });
        }
    ...
    $(document).click(function(event){
        if (isSidebarOpen && !$(event.target).closest('#principal-collapsable').length && !$(event.target).is('#collapse')) {
            sidebar.stop().animate({ 'right': '-250px' }, 'slow', function() {
