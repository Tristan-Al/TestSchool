    ...
    <form action="{{ route('confirmEditSubject') }}" method="post">
        ...
        <select name=formation_id id="formation_id">
            @foreach ($formations as $formation)
                <option value="{{ $formation->id }}" <?php if ($formation->id == $subject->formation_id) {
                    echo 'selected';
        ...
        <input type="text" placeholder="Denomination" name="denomination" id="denomination" maxlength="60"
            value="{{ $subject->denomination }}"><br>
        <label for="acronym">Acronym</label>
        <input type="text" placeholder="Acronym" name="acronym" id="acronym" maxlength="10"
            value="{{ $subject->acronym }}"><br>
        <label for="year">Year</label>
        <select name=year id="year">
            <option value="1" <?php if ($subject->year == '1') {
                echo 'selected';
            } ?>>1st</option>
            <option value="2" <?php if ($subject->year == '2') {
                echo 'selected';
            } ?>>2nd</option>
            <option value="3" <?php if ($subject->year == '3') {
                echo 'selected';
            } ?>>3rd</option>
            <option value="4" <?php if ($subject->year == '4') {
                echo 'selected';
            } ?>>4th</option>
        </select>
